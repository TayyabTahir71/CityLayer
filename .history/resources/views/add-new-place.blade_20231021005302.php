...
<div class="p-6 lg:p-12">
    ...
    <div class="parentDiv relative pt-20" x-cloak x-data="{ tab: '{{$type}}', showMore: false }" :class="tab === 'place' ? 'placeTab' : 'observationTab'">
        <div class="flex items-center justify-center">
            <div class="-mr-1 cursor-pointer" x-show="tab === 'place'">
                <div class="flex flex-col w-[84px] justify-center items-center">
                    <div class="bg-[#2d9bf0] border-2 border-white rounded-full" :class="tab === 'place' ? 'bg-[#2d9bf0] z-10 p-[26px] placeTabActive' : 'bg-[#2d9bf0]/70 p-[40px]'">
            ...
            <div class="-ml-1 cursor-pointer" x-show="tab === 'observation'">
                <div class="flex flex-col w-[75px] justify-center items-center">
                    <div class="flex items-center justify-center border-2 border-white rounded-full" :class="tab === 'observation' || tab === 'observation1' ? 'bg-[#ffa726] z-10 p-[20px] observationTabActive' : 'bg-[#ffa726]/70 p-[40px]'">
                        <div class="flex items-center justify-center w-10 h-10 text-2xl" :class="tab === 'observation' || tab === 'observation1' ? 'block' : 'hidden'" id="observation" alt">🔍</div>
                    </div>
                </div>
            </div>
        </div>

        <div class="placeSubTab flex flex-col items-center justify-center" x-data="{ placeSubtab: 'add', searchQueryPlace: '', placeTitle: 'Add new place', placeInput: 'Enter a new place', active: '' }" x-show="tab === 'place" :class="placeSubtab === 'choose' ? 'choose' : 'add'">
            <button class="hidden openPlaceModel" @click="tab = 'place'; placeSubtab = 'choose'; placeTitle = 'Choose tags or add a new place'; placeInput = 'Browse place tags'"></button>
            ...
            <div class="grid grid-cols-4 italic font-semibold gap-10" x-show="placeSubtab === 'choose'">
                <div class="flex flex-col items-center cursor-pointer">
                    <button @click="placeSubtab = 'add'; placeTitle = 'Add new place'; placeInput = 'Enter a new place'">
                        <div class="rounded-full border-site border-4 p-[22px] bg-site">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="#ffff" class="w-6 h-6 font-bold color-site">
                ...
                @php
                $placeLowerCase = strtolower($place->name);
                @endphp
                <div x-show="(showMore || {{ $key }} < 2) && (searchQueryPlace === '' || '{{ $placeLowerCase }}'.includes(searchQueryPlace.toLowerCase()))" class="flex flex-col items-center cursor-pointer" @click="active='PL_{{ $place->id }}'">
                    <div data-place="{{ json_encode(['id' => $place->id, 'name' => $place->name, 'child' => $place->subplaces]) }}" class="w-[76px] h-[76px] rounded-full bg-[#2d9bf0]" :class="active === 'PL_{{ $place->id }}' ? 'border-4 border-blue-300 placeActive' : ''">
                ...
                <div x-show="!showMore" class="cursor-pointer">
                    <button @click="showMore = true; height = '100vh'">
                        <div class="rounded-full border-site border-4 p-[22px]">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6 font-bold color-site">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m6-6H6" />
                            </svg>
                        </div>
        ...
        <div class="observationSubTab flex flex-col items-center justify-center" x-data="{ observationSubtab: 'add', searchQueryObservation: '', observationTitle: 'Add new observation', observationInput: 'Enter a new observation', active: '', observationModel: 'parent' }" x-show="tab === 'observation'" :class="observationSubtab === 'choose' ? 'choose' : 'add'">
            ...
            <div class="ObservationItems w-full" x-show="observationModel === 'parent'">
                ...
                <div class="flex items-start justify-center gap-10 mt-6 mb-6 italic font-semibold">
                    <div class="grid grid-cols-4 italic font-semibold gap-10" x-show="observationSubtab === 'choose'">
                        <div class="flex flex-col items-center cursor-pointer">
                            <button @click="observationSubtab = 'add'; observationTitle = 'Add new observation'; observationInput = 'Enter a new observation'">
                                <div class="rounded-full border-site border-4 p-[22px] bg-site">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="#ffff" class="w-6 h-6 font-bold color-site">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m6-6H6" />
                                    </svg>
                        ...
                        @foreach($allObservations as $key => $observation)
                            @php
                            $observationLowerCase = strtolower($observation->name);
                            @endphp
                            <div x-data="{ active: false }" x-show="(showMore || {{ $key }} < 2) && (searchQueryObservation === '' || '{{ $observationLowerCase }}'.includes(searchQueryObservation.toLowerCase()))" class="flex flex-col items-center cursor-pointer" @click="active = !active ? 'OB_{{ $observation->id }}' : false">
                                <div data-observations="{{ json_encode(['id' => $observation->id, 'name' => $observation->name, 'child' => $observation->subobservs]) }}" class="w-[76px] h-[76px] rounded-full bg-[#ffa726]" :class="active === 'OB_{{ $observation->id }}' ? 'border-4 border-blue-300 observationActive' : ''">
                                    <div class="flex align-item-center justify-center items-center h-full">
                        ...
                        <div x-show="!showMore" class="flex flex-col items-center justify-center cursor-pointer">
                            <button @click="showMore = true; height = '100vh'" type="button">
                                <div class="rounded-full border-[#ffa726] border-4  p-[22px]">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6 font-bold text-[#ffa726]">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m6-6H6" />
                                    </svg>
                                </div>
                                <div class="mt-4 text-black font-normal">See more</div>
            ...
            <div class="mt-12 mb-12" x-show="observationModel === 'child'">
                <div  class="grid grid-cols italic font-semibold gap-10 subObservationItems">
                </div>
            </div>
            <div x-show="observationSubtab === 'add'" x-data="{ feelActive: 'feel__1' }" class="flex flex-wrap items-center justify-center gap-2 italic font-semibold w-full ">
                @foreach($feelings as $feeling)
                    <div data-feeling_id="{{$feeling->id}}" @click="feelActive='feel_{{ $feeling->id }}'" class="cursor-pointer rounded-full" :class="feelActive === 'feel_{{ $feeling->id }}' ? 'border-4 border-blue-300 feelingActive' : ''">
...
<script>
        ...
        $('body').on('click', '.submitDataAll', function() {
            ...
            if (parentDiv.hasClass('observationTab')) {
                ...
                } else if (parentDiv.find('.' + tab + 'SubTab').hasClass('choose')) {
                ...
                success: function(data) {
                    ...
                    if (data.status === 'success') {
                        if (data.completed) {
                            ...
                            swal({
                                title: "Success!",
                                text: data.tab.charAt(0).toUpperCase() + data.tab.slice(1) + " " + data.msg + " added successfully!",
                                icon: "success",
                                button: "OK",
                            }).then(function() {
                                $('.Open' + data.tab.charAt(0).toUpperCase() + data.tab.slice(1) + 'Model').click();
                                place_data['tab'] = reverse_tab;
                            });
                        }
                    } else {
                        swal({
                            title: "Fail!",
                            text: data.tab.charAt(0).toUpperCase() + data.tab.slice(1) + " " + data.msg + " added failed!",
                            icon: "error",
                            button: "OK",
                        });
                    }
        ...
        $('.place_image').on('change', function() {
            const file = $(this)[0].files[0];
            const url = URL.createObjectURL(file);
            const parentDiv = $(this).closest('.parentDiv');
            parentDiv.find('.place_image_preview').show();
            parentDiv.find('.place_image_preview').attr('src', url);
        });
        $('.observation_image').on('change', function() {
            const file = $(this)[0].files[0];
            const url = URL.createObjectURL(file);
            const parentDiv = $(this).closest('.parentDiv');
            parentDiv.find('.observation_image_preview').show();
            parentDiv.find('.observation_image_preview').attr('src', url);
        });
        $('body').on('click', '.openPlaceModel', function() {
            submitDetailData(place_data);
        });
        $('body').on('click', '.openObservationModel', function() {
            submitDetailData(place_data);
        });
</script>
</div>
