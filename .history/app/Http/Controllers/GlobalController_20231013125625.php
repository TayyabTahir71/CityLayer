...
class GlobalController extends Controller
{
    ...
    public function addMapPlace(Request $request, $id = null)
    {
        ...
        $place = PlaceDetails::selectRaw('*, (6371 * acos(cos(radians(?)) * cos(radians(latitude)) * cos(radians(longitude) - radians(?)) + sin(radians(?)) * sin(radians(latitude))) as distance')
            ->where('user_id', '=', $userId)
            ->having('distance', '<=', $radiusMeters / 1000) // Convert meters to kilometers
            ->orderBy('distance')
            ->setBindings([$latitude, $longitude, $latitude])
            ->first();

            $place = DB::table("spots")
            ->select("*"
