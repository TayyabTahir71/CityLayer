...
class GlobalController extends Controller
{
    ...
    public function getAll()
    {
        ...
        if (backpack_auth()->check()) {
            ...
            if (Infosperso::where('user_id', $userid)->exists()) {
                ...
                $query = PlaceDetails::where('user_id', $userid)
                    ->with('place', 'observation', 'user');

                if (session()->has('placeIds') && count(session('placeIds')) > 0) {
                    $placeIds = session('placeIds');
                    $query->whereIn('place_id', $placeIds);
                }

                if (session()->has('observationIds') && count(session('observationIds')) > 0) {
                    $observationIds = session('observationIds');
                    $query->orWhereIn('observation_id', $observationIds);
                }
                ...
                $feeling_list = [
                    ...
                    ['name'=>'Very good','star'=>'4','image'=>'/img/confused.png'],
                    ['name'=>'Good','star'=>'3','image'=>'/img/sad.png'],
                    ['name'=>'Average','star'=>'2','image'=>'/img/happy.png'],
                    ['name'=>'Bad','star'=>'1','image'=>'/img/disgusted.png'],
    ...
    public function community_achievements()
    {
        ...
        $usersWithTotals = User::withCount([
            'placeDetails as total_places' => function ($query) {
                $query->whereNotNull('place_id');
            },
            'placeDetails as total_observations' => function ($query) {
                $query->whereNotNull('observation_id');
            },
    ...
    public function preferences()
    {
        ...
        $preferences_array = Preference::getPreferences();
    ...
    {
        // dd($request->child_place_id);

        $userId = backpack_auth()->user()->id;
        $response = ['status'=>'','msg'=>'','place_detail_id'=>'','tab'=>'','completed'=>false];

        if(isset($request->place_detail_id) && $request->place_detail_id!=''){
            $place = PlaceDetails::find($request->place_detail_id);
        }else{
            $latitude = $request->latitude;
            $longitude = $request->longitude;
            $radius = 100; // Meters
            $place = PlaceDetails::selectRaw("*,
        ...
        if (isset($place)) {
            $place->update([
                ...
                'place_child_id' => $request->child_place_id,
                'observation_id' => $request->observation_id,
                'child_observation_id' => $request->child_observation_id,
            ]);
            $response['status'] = 'success';
            $response['msg'] = 'data updated successfully!';
        } else {
            $place=PlaceDetails::create([
                'place_id' => $request->place_id,
                'place_child_id' => $request->child_place_id,
                'observation_child_id' => $request->child_observation_id,
        ...
        $response['place_detail_id'] = $place->id;

        
        if($request->place_id && $request->observation_id){
            $response['completed'] = true;
        }
            
        $response['tab'] = $request->tab;
