...
class Filter extends Component
{
    ...
    ->whereIn('place_id', Place::pluck('id')->toArray())
    ->orWhereIn('observation_id', Observation::pluck('id')->toArray())
    ->orderBy('id', 'desc')
    ->get();
    ...
    $uniquePlaces = $sortedPlaceDetails->groupBy(function ($item) {
        return $item->place_id ?: $item->observation_id;
    })->map(function ($group) {
        return $group->first();
    });
