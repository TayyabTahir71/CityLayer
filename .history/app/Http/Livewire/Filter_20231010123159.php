...
class Filter extends Component
{
    ...
    public function updateMap()
    {
        ...
        foreach($this->formData as $key=>$value){
            ...
            if (strpos($key, 'place_') === 0 && $value!=false) {
                $this->selected_places['place'][]=(int)str_replace('place_', '', $value);
            }elseif(strpos($key, 'observation_') === 0 && $value!=false){
                $this->selected_places['observation'][]=(int)str_replace('observation_', '', $value);
            }
