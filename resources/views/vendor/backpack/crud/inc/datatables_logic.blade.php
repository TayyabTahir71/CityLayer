  ...
  <script>
    ...
    window.crud = {
      ...
      updateUrl : function (url) {
        let urlStart = "{{ url($crud->route,[],$https) }}";
      ...
      dataTableConfiguration: {
        ...
        */
        ...
        language: {
          ...
          ajax: {
              "url": "{!! url($crud->route.'/search',[],$https).'?'.Request::getQueryString() !!}",
              "type": "POST",
              "data": {
  ...
  <script type="text/javascript">
    jQuery(document).ready(function($) {
      ...
      @if($crud->getOperationSetting('resetButton') ?? true)
        ...
        var crudTableResetButton = '<a href="{{url($crud->route,[],$https)}}" class="ml-1" id="crudTable_reset_button">{{ trans('backpack::crud.reset') }}</a>';
